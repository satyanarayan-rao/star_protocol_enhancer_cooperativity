...
for line in inp_fp:
    ...
    # new example line: head -2 mnase_peaks_in_open_enh.bed
    # chr2L	107902	107903	peak_2105_9	open	+
    # chr2L	108052	108053	peak_2105_1	open	+
    l_items = line.strip().split() 
    peak_id_without_offset = "_".join(l_items[3].split("_")[0:2]) 
    peak_list_dict[peak_id_without_offset].append(l_items[3])
    chr_loc = int(l_items[1])
    chr_loc_dict[peak_id_without_offset].append(chr_loc)
    peak_chr[peak_id_without_offset] = l_items[0]
            ...
            if abs(chr_loc_dict[peak][i] - chr_loc_dict[peak][j]) >= th:
                if chr_loc_dict[peak][j] > chr_loc_dict[peak][i]: 
                    print ("\t".join([peak_chr[peak],
                                      ...
                                      peak_chr[peak] + ":" + str(chr_loc_dict[peak][i]) + "-" + str(chr_loc_dict[peak][i]), 
                                      "+",
                                      "0", 
                    ...
                    print ("\t".join([peak_chr[peak],
                                      ...
                                      peak_chr[peak] + ":" + str(chr_loc_dict[peak][j]) + "-" + str(chr_loc_dict[peak][j]), 
                                      "+",
                                      "0", 
