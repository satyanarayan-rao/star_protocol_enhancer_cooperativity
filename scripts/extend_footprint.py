...
roi_dict = pickle.load(open(sys.argv[3], "rb"))
lflank = int(sys.argv[4])
rflank = int(sys.argv[5])
lextend = int(sys.argv[6])
rextend = int(sys.argv[7])

out_fp = open(sys.argv[8], "w")
out_verb_fp = open(sys.argv[9], "w")
for line in input_fp: 
    key_with_hash = line[0:line.find("\t")]
    roi_id = key_with_hash.split("`")[0]
    key_without_hash = key_with_hash.split("#")[0]
