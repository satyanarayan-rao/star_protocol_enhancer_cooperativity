...
footprint_dict = pickle.load(open(sys.argv[3], "rb")) 
bed_fp_for_strand_info = open(sys.argv[4])
out_fp_per_orange_and_flen = open(sys.argv[5], "w")
out_fp_percentage_methylation =  open(sys.argv[6], "w")
out_fp_percentage_methylation_pkl = open(sys.argv[7], "wb")
# for neighborhood analysis
extend_left_from_center = int(sys.argv[8]) 
extend_right_from_center = int(sys.argv[9]) 
lflank = int(sys.argv[10])
rflank = int(sys.argv[11]) 
per_methylation_dict = defaultdict(dict)

strand_dict = {}
roi_center_dict = {} 
for line in bed_fp_for_strand_info:
    # example line: head -2 input_bed/example.bed
    # chr2L	480305	480306	peak_229	.	+
    # chr3L	616610	616611	peak_613	.	+
    d_loc = [m.start() for m in re.finditer("\t", line)]
    roi_id = line[d_loc[2] + 1:d_loc[3]]
    roi_center_dict[roi_id] = line[d_loc[0] + 1: d_loc[1]]
    if len(d_loc)  > 5:
        s = line[d_loc[4]+1:d_loc[5]]
        strand_dict[roi_id] = s 
    else:
        s = line[d_loc[4]+1:].strip() 
        strand_dict[roi_id] = s
bed_fp_for_strand_info.close() 
...
for line in inp_fp:
    ...
    k = line[0:d_loc[0]]
    roi_id = k.split("`")[0]
    pc = roi_center_dict[roi_id]
    strand = strand_dict[roi_id]
    ...
    _, left_neighbor, _ = extend_footprint_from_center(line, footprint_dict,
                            0, 0, extend_left_from_center, rflank, strand, pc) #  
    _, right_neighbor, _ = extend_footprint_from_center(line, footprint_dict,
                            0, 0, lflank, extend_right_from_center, strand, pc) # 
